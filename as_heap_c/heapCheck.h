...
typedef struct checkOutput {
    ...
    union {
        struct {
            ...
            int externalFragments;
        }fragment;
        bool isOverrun;
    };
} checkOutput_t;

//parameter adresses are not limited to malloc or realloc returns. 

//-----Check if the Heap is Coalesced-----//
bool _isHeapCoalesced(void* heap); //need more robust return type //change name for heap to ptr
...
checkOutput_t _amountOfInternalFragments(void* heap); //need more robust return type //change name for heap to ptr
...
checkOutput_t _amountOfExternalFragments(void* heap); //change name for heap to ptr


//-----Check if the heap overruns its specified block of memory-----//
checkOutput_t _checkHeapOverun(void* heap); //change name for heap to ptr
